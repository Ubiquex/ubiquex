...
func ResourceFile(pkgName, localWireName string, rt *ir.ResourceType, configTypeNameOverride string) (string, error) {
	...
	// sdk.FieldMap var as a side effect (r.fieldMapDecls -- see
	...
	fmt.Fprintf(&descriptor, "var %s = sdk.ResourceBinding{\n", pascalName)
	fmt.Fprintf(&descriptor, "\tWireType: %q,\n", rt.WireType)
	descriptor.WriteString("\tFields: sdk.FieldMap{\n")
	for _, f := range rt.Fields {
		if !fieldIsSettable(f) {
			continue
		}
...
// sdk.FieldMap{...} literal into ONE shared top-level `var`
...
// which may hoist a shared top-level sdk.FieldMap var as a side effect
...
// sdk.FieldMap -- either a reference to an already-hoisted shared
...
// fresh inline sdk.FieldMap{...} literal that ALSO gets hoisted into a
...
// objectFieldMapLiteral renders an INLINE sdk.FieldMap literal for one
