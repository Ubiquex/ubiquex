...
func newSDKGenCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "gen",
		Short: "Generate typed TypeScript bindings for every provider declared in [providers] -- local, never published",
...
credentials needed -- a pure local gRPC call against the launched binary), and writes one TypeScript file per
provider source under --out: idiomatic camelCase bindings whose generated toConfig-style field map maps back to
the provider's real wire attribute names at evaluation time.
...
generated code (docs/sdk.md); re-run this command after bumping a provider's pinned version in [providers].`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			for _, source := range sortedProviderSources(cfg.Providers) {
				...
				path, count, err := generateOneProvider(cmd.Context(), timeout, source, version, out)
				if err != nil {
					return &ExitCodeError{Code: 2, Err: fmt.Errorf("sdk gen: %w", err)}
				}
	...
	cmd.Flags().StringVar(&out, "out", "sdk/generated", "directory to write generated TypeScript bindings into (one file per declared provider source)")
...
// schema, runs it through sdk/codegen/ir + the TS template, and writes
// the result to <out>/<source-sanitized>.ts. A separate function (not
// inlined into RunE) so a per-provider context.WithTimeout is easy to
// scope correctly -- each declared provider gets its own fresh timeout
// budget, never one shared budget that a slow first provider could eat
// into a fast second one's own allowance.
func generateOneProvider(ctx context.Context, timeout time.Duration, source, version, out string) (path string, resourceCount int, err error) {
	...
	path = filepath.Join(out, sanitizeSourceForFilename(source)+".ts")
