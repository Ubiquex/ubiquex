...
| 11 | `$cross` by stack name, no base to resolve against | `$cross`'s inner object uses `"stack": "networking"` (UBI-32 Arc B's own addressing amendment) instead of `"ledger_dir"`, but the current stack's own `.ubx/config` `[ledger]` store is absent/`"git"` (git-local) and no `[ledger.external]` entry names `"networking"` either. | Resolve fails hard (`ErrRefNotFound`), naming the stack and explicitly suggesting `"ledger_dir"` as the git-local alternative — never silently treated as if `"stack"` were simply absent, never a nil/empty pin. |
| 12 | `$cross` names both `"ledger_dir"` and `"stack"` at once | A single `$cross` inner object sets both keys, a structurally contradictory input (which one wins is never something to guess at). | Resolve fails hard (`ErrRefNotFound`), naming that exactly one of the two is allowed — never silently preferring one over the other. |
| 13 | `$cross` by stack name, resolved against a real remote neighbor, staleness caught | Two stacks share one real base store; one pins the other via `"stack"` (not `"ledger_dir"`); after resolve, a real proposal is accepted against the neighbor, advancing its head. | Identical claim to row 5, now proven for a remote neighbor specifically, not just git-local: `VerifyPins` (`core.OpenRef`-based, format-blind) still catches the staleness — the by-name addressing mechanism is never a separate code path from the explicit-`ledger_dir` one once resolution has recorded a concrete address into `resolution.inputs`. |
| 14 | Resource config carries a hallucinated attribute key | A resource intent's `config` uses a plausible-sounding but non-existent key for its real type (e.g. `"repository_name"` for `aws_ecr_repository`'s real `"name"`; `"role_name"`/`"assume_role_policy_document"` for `aws_iam_role`'s real `"name"`/`"assume_role_policy"`) — found live, UBI-66, a real Claude Haiku draft. | Resolve fails hard (`ErrUnknownConfigKey`), naming every offending key across the WHOLE batch in one refusal (not just the first), each with a fuzzy-match suggestion when a real key at that level is close enough ("config key \"role_name\" does not exist on aws_iam_role (did you mean \"name\"?)") — never silently dropped, never reaching a real `ApplyResourceChange` call to fail there instead, potentially after other resources in the same batch already applied. Checked recursively (nested blocks too, not just top-level attributes), against the exact schema `InferProvider` already fetched for type inference. Model-agnostic: the identical check runs whether the `ResourceIntent` came from a hand-written file, an evaluated SDK program, or any `intentprovider.Adapter`'s own draft — resolve has no provenance information to branch on. |
