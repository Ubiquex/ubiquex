...
| 13 | PR-ceremony mirror: merge without the proposal file | `ubx accept --from-merge` is invoked against a merge commit that never actually carries the proposal file at `--proposal-file`'s own path (a bad path, or a merge that genuinely never included it). | Refused before `openLedgerForStack` is ever called — `ghub.DeriveAcceptance`'s own git/GitHub verification fails first, so the configured remote store is never opened, let alone written to. Confirmed directly: the store ends up with zero objects, not a partially-initialized one. |
| 14 | PR-ceremony mirror: tampered/mismatched trailer | The merged proposal file's own content doesn't hash to the PR body's claimed `ubx-proposal:` trailer (a tamper, or an authoring bug) — `--from-merge` targets a stack whose `[ledger]` names a remote store. | `core.AcceptFromMerge`'s own `validateAndHash` step (comparing the recomputed hash against the claimed one) runs, and fails, *before* `Ledger.Append` ever reaches the store — confirmed directly: zero objects ever land in the configured bucket, the identical `ErrTrailerHashMismatch` the git-local path already reports. |
| 15 | PR-ceremony mirror: CAS conflict on the real system of record | Two merge-derived proposals, both recording the same stale `Parent` (each authored/reviewed independently, unaware of the other), are `--from-merge`-accepted in sequence against the same remote-store stack. | The first lands cleanly, advancing the store's real head. The second is refused with the same `ErrParentMismatch` a git-local CAS conflict already produces — never silently appended on top of the now-wrong parent, never corrupting the store's own head. The store's head after both attempts is exactly the first (winning) proposal's ID. |
| 16 | PR-ceremony mirror: re-accept idempotency | The identical merge commit is `--from-merge`-accepted twice against the same remote-store stack (a re-run CI job, an operator re-triggering a merge event). | The second call recomputes the identical proposal (same git history, same content, same ID) and resolves exactly like local accept's own idempotency contract: the object already exists and the head has already advanced past it — a genuine duplicate, `ErrDuplicateProposal`, never silently re-mirrored, re-appended, or treated as a fresh acceptance. |
...
`ubx ship`/`ubx accept` (both local and `--from-merge`, PR-ceremony
mirroring now built, see rows 13-16 above) all now read `.ubx/config`'s
own `[ledger]` table and enforce it, live-verified against real S3
(a real two-stack `$cross`-by-name cross-stack pin, `VerifyPins`
catching a real neighbor advancement; a real GitHub PR opened and merged
into `Ubiquex/ubiquex-cli` itself, `accept --from-merge` genuinely
mirroring the accepted record into a real S3 bucket, verified directly
via `aws s3api get-object`, then fully reverted/cleaned up afterward)
— see STATE.md for the session this landed in and exactly which CLI
surface, if any, still doesn't
(`ubx revert-plan`/`ubx writeback`/`ubx propose`/the MCP surface,
named there rather than silently left unstated).
