...
| 12 | Provider reports a clean destroy success, but never actually deletes (UBI-44) | `ApplyResourceChange` returns nil error and the correct literal `null` `NewState` — byte-for-byte the shape a genuine destroy produces, zero diagnostics — but the resource is still reachable on the very next `ReadResource`. Found live against a real `google_pubsub_topic`, confirmed via Cloud Audit Logs showing zero real `DeleteTopic` calls despite this identical response. | Never resolves `applied`/`destroyed` on the strength of the clean response alone. The universal post-destroy read-back (docs/executor.md's own amendment) finds the resource present and, once the retry budget genuinely exhausts, resolves a definitive `failed`, `Outcome: "provider_reported_success_but_present"` — a distinctly-worded, more serious finding than an ordinary ambiguous-RPC `failed`, since it means the provider actively lied. A re-ship against a still-lying provider fails again, never accepting the claim on a later attempt either. |
| 13 | Genuine destroy, bounded propagation lag, provider claimed success (UBI-42) | Same clean-success `ApplyResourceChange` response as row 12, but the resource genuinely does become absent — after a few further `ReadResource` calls, not immediately — modeling real cloud eventual consistency (SQS's own documented ~60-second deletion-visibility lag, UBI-30's own live finding) rather than either an instant delete or a permanent lie. | Resolves `applied`/`Outcome: "destroyed"` once the read-back genuinely finds it absent, retried through the intervening presence via `Outcome: "inconclusive"` entries rather than giving up on the very first still-present read (row 12's own immediate-failure behavior is deliberately *not* applied here — presence right after a claimed success is not yet conclusive the way presence after a genuinely ambiguous RPC failure is, since real lag can look identical to a lie until the budget's own tail is reached). |
| 14 | Honest, synchronously-consistent destroy costs nothing extra (UBI-44/42) | The ordinary, honest case: `ApplyResourceChange` reports success and the resource is genuinely, immediately absent on the very first post-destroy read — real behavior confirmed live for GCP Pub/Sub once UBI-44's fix was in place. | Resolves `applied`/`Outcome: "destroyed"` on the very first reconcile-by-query attempt, with no added sleep or retry — exactly 2 `Reconciliation` entries total (the pre-attempt `present_matches` precheck, then `destroyed`), identical in shape and cost to what an unconditionally-trusted clean response used to produce. The universal read-back's own cost is paid only by a genuinely slow or lying tail (rows 12/13), never by the common case. |
