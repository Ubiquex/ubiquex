...
// its own config) that an in-process call structurally can't. Returns
// canonical (RFC 8785/JCS-style, core.CanonicalJSONBytes) intent/v1
// bytes on success; core.ErrDoubleRunMismatch if the two runs disagree;
// a structural-validation error (validate.go) if the result doesn't
// match ubx:intent/v1's own real shape.
func Evaluate(ctx context.Context, entryFile string) ([]byte, error) {
	...
	if err := validateIntentShape(canon); err != nil {
		return nil, err
	}
	return canon, nil
}
