...
func TestWriteMergedImportMap_KeepsTheProjectsOwnEntries(t *testing.T) {
	...
	path, cleanup, err := writeMergedImportMap(dir, "/assets/runtime/src/index.ts", nil, nil)
	if err != nil {
		t.Fatal(err)
	}
	defer cleanup()
...
func TestWriteMergedImportMap_SchemedTargetsAreUntouched(t *testing.T) {
	...
	path, cleanup, err := writeMergedImportMap(dir, "/rt.ts", nil, nil)
	if err != nil {
		t.Fatal(err)
	}
	defer cleanup()
...
func TestWriteMergedImportMap_NoProjectConfig(t *testing.T) {
	path, cleanup, err := writeMergedImportMap(t.TempDir(), "/rt.ts", nil, nil)
	if err != nil {
		t.Fatal(err)
	}
	defer cleanup()
