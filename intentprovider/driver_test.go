...
func TestDraftWithRetry_SucceedsFirstAttempt(t *testing.T) {
	...
	draft, raw, err := DraftWithRetry(context.Background(), a, "payments", []byte("some doc"), nil, nil)
	if err != nil {
...
func TestDraftWithRetry_RecoversOnSecondAttempt(t *testing.T) {
	...
	draft, _, err := DraftWithRetry(context.Background(), a, "payments", []byte("some doc"), nil, nil)
	if err != nil {
		t.Fatalf("DraftWithRetry: %v", err)
	}
	if draft == nil {
...
func TestDraftWithRetry_AdapterErrorAbortsImmediately(t *testing.T) {
	...
	_, _, err := DraftWithRetry(context.Background(), a, "payments", []byte("some doc"), nil, nil)
	if err == nil {
...
func TestDraftWithRetry_DuplicateResourceAddressRejected(t *testing.T) {
	...
	_, _, err := DraftWithRetry(context.Background(), a, "payments", []byte("doc"), nil, nil)
	if err == nil {
