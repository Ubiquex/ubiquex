...
func (g *tsGenerator) renderAny(v any) (string, error) {
	...
	case map[string]any:
		...
		if name, _, ok := fnCallMarker(t); ok {
			...
			return "", fmt.Errorf("blueprint: this draft uses the ported function %q, which is only supported in Go so far -- build --lang go, or wait for a TypeScript port", name)
		}
...
func renderTSBindings(blueprintName string, resources []*tsResource) string {
	var b strings.Builder
	...
	for _, tr := range resources {
		...
		b.WriteString("  },\n")
		// blueprintName (UBI-225) is the SAME raw name renderTSFunction's
		// own generated wrapper function passes to pushBlueprintSource --
		// carried on the binding itself so a resource() call made
		// directly against this binding, bypassing the wrapper function
		// entirely, still gets real provenance.
		fmt.Fprintf(&b, "  blueprintName: %s,\n", jsonStringLiteral(blueprintName))
		b.WriteString("};\n\n")
	}
	return b.String()
}
...
//
// blueprintName (UBI-126, mirroring GenerateGo's own identical
// parameter) wraps the function body in
// pushBlueprintSource(blueprintName)/popBlueprintSource() (a try/finally,
// TS's own equivalent of Go's defer) -- the SAME raw, unsanitized name
// (never pkgName, a TS-identifier-sanitized derivative)
// buildManifest/ubx why/ubx render already use, so every resource() call
// inside this function -- direct SDK import, the ONLY calling convention
// that previously had no provenance mechanism at all (diagram/md get
// theirs externally, from blueprint.ExpandCalls) -- self-tags without
// the calling stack's own code needing to change in any way.
func renderTSFunction(funcName, blueprintName string, params []Param, g *tsGenerator) (string, error) {
	...
	var b strings.Builder
	b.WriteString("// Code generated by `ubx blueprint build`. DO NOT EDIT.\n")
	b.WriteString("import { popBlueprintSource, pushBlueprintSource, resource")
	if usesAddressOf(g) {
	...
	fmt.Fprintf(&b, "  pushBlueprintSource(%s);\n", jsonStringLiteral(blueprintName))
	b.WriteString("  try {\n")
	...
	// deeper when spliced into the try{} block below -- keeps every
	// existing rendering line above completely unchanged (no per-line
	// indentation bookkeeping threaded through the loop itself).
	...
	for _, tr := range g.topoOrdered() {
		if tr.dr.ForEach != "" {
			...
			call.WriteString("    })")
			fmt.Fprintf(&body, "  const %s: any[] = [];\n", forEach.accumIdent)
			fmt.Fprintf(&body, "  %s.forEach((%s, %s) => {\n", forEach.paramIdent, forEach.valueIdent, forEach.indexIdent)
			fmt.Fprintf(&body, "    const item = %s;\n", call.String())
			fmt.Fprintf(&body, "    %s.push(item);\n", forEach.accumIdent)
			body.WriteString("  });\n")
			continue
		}
		...
		if g.blueprint.Referenced[tr.dr.Address] {
			...
			fmt.Fprintf(&body, "  const %s = %s as any;\n", varName, call.String())
		} else {
			fmt.Fprintf(&body, "  %s;\n", call.String())
		}
	}
	...
	b.WriteString("    popBlueprintSource();\n")
	b.WriteString("  }\n")
	b.WriteString("}\n")
	return b.String(), nil
}

// indentLines prefixes every non-empty line of s with prefix -- used to
// re-indent an already-rendered function body one level deeper when
// wrapping it in a try/finally (TS, renderTSFunction) or try/finally
// (Python, renderPyFunction) block for UBI-126's own blueprint-source
// push/pop scope. A blank line stays blank (no trailing whitespace
// introduced).
