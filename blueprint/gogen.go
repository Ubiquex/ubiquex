...
func renderGoBindings(pkgName, blueprintName string, resources []*goResource) string {
	var b strings.Builder
	...
	for _, gr := range resources {
		...
		// BlueprintName (UBI-225) is this blueprint's own bare declared
		// name -- the SAME value renderGoFunction's own generated
		// wrapper function passes to sdk.PushBlueprintSource. Carrying
		// it on the binding itself, not just the wrapper function's own
		// call scope, means a resource built by calling
		// sdk.Resource(<this binding>, name, <this config>) directly --
...
func renderGoFunction(pkgName, funcName, blueprintName string, params []Param, g *goGenerator) (string, error) {
	...
	for _, gr := range g.topoOrdered() {
		...
		if gr.dr.CreateIf != "" {
			...
			cond, err := g.paramRef(gr.dr.CreateIf)
			if err != nil {
