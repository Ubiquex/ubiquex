...
// --- validate.go: change proposals reject destroys ------------------------

func TestValidate_ChangeProposalWithDestroys_Rejected(t *testing.T) {
	l := core.Open(t.TempDir())
	schema := newFakeSchema()
	intent := intentFile("payments",
		ri("aws_vpc", "main", OpCreate, `{}`),
	...
	p.Delta.Destroys = []core.Address{{Stack: "payments", Type: "aws_vpc", Name: "old"}}
	if err := core.Validate(p); err == nil {
		t.Fatal("expected an error for a change proposal carrying destroys")
	}
}
